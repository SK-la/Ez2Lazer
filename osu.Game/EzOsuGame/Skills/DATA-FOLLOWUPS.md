...
| — | DATA-ChartDan-Keymode-Routing | 5K / 8K–18K have no community xxy→dan table, so their chart dan is star-fitted: `EzDanLabels.FitTablelessRawDan` (`Overall MSD × xxySR × 0.05`) replaces the raw 4K MSD means table that inflated those keymodes by several levels. 4/6/7K keep Sunny tables + the calibrated means fallback. No star rating → no invented dan (label stays empty). `EzDanAlgorithm.VERSION` 3→4 invalidates old rows; re-run Local Profile → Skills to see the new values. | **Done** |
