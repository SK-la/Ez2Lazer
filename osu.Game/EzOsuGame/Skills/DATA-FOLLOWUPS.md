...
| — | DATA-PlayerSkillChain | 玩家链与图表链分开：一局结算即在 `Player.ImportScore` 追加 SQLite 切片（`drill_scores` 账本）并把 Realm 技能行置 `Stale`，**不**在结算路径重算；`EzLocalProfileStartupAlign` 用「账本缺成绩 / 脏位 / 内容版本」三信号做一次增量对账（`PlanStartupAlign` + `AlignOnStartupAsync`），**不再在启动时自动跑**（仅手动计算报缺后由图表链回折触发），无事可做则零开销零通知。手动「计算成绩分析」自己读脏位作为默认技能范围（`ComputeAsync` 未传 `skillsUsernames` 时），一遍覆盖完再按玩家整体清脏位；无法重算的玩家（该切片没有 mania 成绩）直接删行，避免脏位永远清不掉。进度通知统一走 `EzLocalProfileComputeNotification`。不用组合修订号：上游是"可增可删的成绩"，账本 diff 已是精确判据。 | **Done** |
