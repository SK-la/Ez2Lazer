...
namespace osu.Game.EzOsuGame.Skills
{
    ...
    /// <para>
    /// A play waits when its chart is one the chain will rate - the beatmap still exists and its keymode is inside the
    /// engine's range - but carries no current chart-skill row. The two shapes are not equally fixable:
    /// <see cref="WaitingOnCsiByUser"/> charts have a settled MSD and only need the CSI stage to run, while
    /// <see cref="WaitingOnMsdByUser"/> charts have no settled MSD at all, so that stage never yields the row they
    /// need and they stay in debt until it succeeds.
    /// </para>
    /// <para>
    ...
    public sealed class EzChartChainDebt
    {
        ...
        public static readonly EzChartChainDebt Empty = new(
        ...
        /// <summary>Waits that cannot end until MSD produces a row for the chart, per player.</summary>
        public IReadOnlyDictionary<string, int> WaitingOnMsdByUser => waitingOnMsd;

        /// <summary>Waits the CSI stage can still answer, per player.</summary>
        ...
        public int PlaysFor(string username)
        {
            ...
            return (waitingOnMsd.TryGetValue(username, out int msd) ? msd : 0)
                   + (waitingOnCsi.TryGetValue(username, out int csi) ? csi : 0);
        }
        ...
        public static EzChartChainDebt Collect(IEnumerable<(string Username, string BeatmapHash)> plays, EzSkillStore store)
        {
            ...
            foreach (var play in plays)
            {
                ...
                bucket[play.Username] = (bucket.TryGetValue(play.Username, out int existing) ? existing : 0) + 1;
            }
