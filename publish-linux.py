...
def _find_best_publish_candidate(search_root: str) -> str | None:
    import glob
    candidates = []
    # prefer explicit bin/Release folders
    for root, dirs, files in os.walk(search_root):
        if os.path.sep + 'bin' + os.path.sep in root and os.path.sep + 'Release' + os.path.sep in root:
            candidates.append(root)
    if not candidates:
        # fallback: glob for bin/Release directories
        candidates = glob.glob(os.path.join(search_root, '**', 'bin', 'Release', '*'), recursive=True)
        if not candidates:
            return None
    # choose candidate with most bytes
    def _dir_size(p):
        total = 0
    ...
    best = max(candidates, key=_dir_size)
    return best
...
def main():
    ...
    if rc != 0:
        ...
        # Diagnostic: list release_dir contents to help CI debugging
        try:
            if os.path.exists(release_dir):
                print('Release directory exists:', release_dir)
                for root, dirs, files in os.walk(release_dir):
                    print('DIR:', root)
                    for f in files[:50]:
                        fp = os.path.join(root, f)
                        try:
                            print('  ', os.path.relpath(fp, release_dir), '-', os.path.getsize(fp))
                        except Exception:
                            print('  ', os.path.relpath(fp, release_dir), '- size-unavailable')
                    break
            else:
                print('Release directory not found:', release_dir)
        except Exception as e:
            print('Failed to list release_dir contents:', e)
            ...
            try:
                ...
                for d in temp_dirs:
                    try:
                        ...
                        pass
            # If release_dir exists and has files, zip it. Otherwise try a fallback candidate.
            ...
            if args.appimage and (target_platform == 'linux' or target_platform == 'linux-x64' or target_platform.startswith('linux')):
                try:
                    ...
                    if rc_tool.returncode != 0:
                        ...
                        # cleanup downloaded tool to avoid being mistaken for generated AppImage
                        try:
