...
namespace osu.Game.Rulesets.Mania.LAsEZMania
{
    public class EzManiaModStrings : EzLocalizationManager
    {
        ...
        public static readonly LocalisableString LoopPlayClip_Description = new EzLocalisableString("将谱面切割成片段用于循环练习。（原版是YuLiangSSS的Duplicate Mod）", "Cut the beatmap into a clip for loop practice. (The original is YuLiangSSS's Duplicate Mod)");
        ...
        public static readonly LocalisableString UseGlobalABRange_Description = new EzLocalisableString("始终使用谱面编辑器中A/B空间设置的范围（毫秒）。推荐保持开启", "Use global A-B range. Always use the editor A/B range stored for this session (ms).");
        ...
        public static readonly LocalisableString InfiniteLoop_Description = new EzLocalisableString("启用无限循环播放。游戏中必须使用Esc退出才能结束，无法获得成绩结算。", "Infinite Loop. Enable infinite loop playback. You must use Esc to exit in the game to end, and you cannot get score settlement.");
        ...
        public static readonly LocalisableString TrueRandom_Description = new EzLocalisableString("随机排列所有音符（使用NoLN（LN转换器等级-3），否则可能会重叠）", "True Random. Shuffle all notes(Use NoLN(LN Transformer Level -3), or you will get overlapping notes otherwise).");
        ...
        public static readonly LocalisableString NtoMAnother_Description = new EzLocalisableString("转Key，来自krrcream的工具（有一些bug，请使用Clean设置来清理）", "From krrcream's Tool (It has some bugs, please use Clean settings to clean it.)");
        ...
        public static readonly LocalisableString DeleteSpace_Description = new EzLocalisableString("让6k玩家使用7k谱面。（但我不知道如何移除中间列）", "For 6k Player to use 7k maps. (But I don't know how to remove middle column.)");
        ...
        public static readonly LocalisableString Key_Description = new EzLocalisableString("目标按键数", "Key. To Keys");

        // JackAdjust
        public static readonly LocalisableString ToStream_Label = new EzLocalisableString("转换为流", "To Stream");
        public static readonly LocalisableString ToStream_Description = new EzLocalisableString("尽可能作为JumpJack（推荐使用中等概率50~80）", "To Stream. As Jumpjack as possible(Recommend to use a medium(50~80) probability).");
        ...
        public static readonly LocalisableString Alignment_Description = new EzLocalisableString("最后一行（false）或第一行（true），true会得到一些子弹，false会得到许多长jack", "Alignment. Last line(false) or first line(true), true will get some bullet, false will get many long jack.");
        ...
        public static readonly LocalisableString ApplyOrder_Description = new EzLocalisableString("此mod在谱面转换后应用的顺序。数字越小越先运行。", "Apply Order. Order in which this mod is applied after beatmap conversion. Lower runs earlier.");
        ...
        public static readonly LocalisableString ReleaseLenience_Description = new EzLocalisableString("调整LN尾部释放窗口宽容度。（Score v2中的尾部默认有1.5倍打击窗口）", "Adjust LN tail release window lenience.(Tail in Score v2 has default 1.5x hit window)");
        ...
        public static readonly LocalisableString NoteAdjustStyle_Description = new EzLocalisableString("1: 适用于Jack模式。2&3: 适用于Stream模式。4&5: 适用于Speed模式（无Jack）。6: DIY（将使用↓↓↓所有选项）（1~5将仅使用↓种子选项）", "1: Applicable to Jack Pattern.  2&3: Applicable to Stream Pattern.  4&5: Applicable to Speed Pattern(No Jack).  6: DIY(Will use ↓↓↓ all options) (1~5 will only use ↓ seed option).");
        ...
        public static readonly LocalisableString Extremum_Description = new EzLocalisableString("取决于你在一行上保留多少音符（可用最大音符或最小音符）", "Depending on how many notes on one line you keep(Available maximum note or minimum note).");
        public static readonly LocalisableString ComparisonStyle_Label = new EzLocalisableString("比较样式", "Comparison Style");
        public static readonly LocalisableString ComparisonStyle_Description = new EzLocalisableString("1: 当此行的音符数量>=上一行和下一行时处理一行。2: 当此行的音符数量<=上一行和下一行时处理一行", "1: Dispose a line when this line's note quantity >= Last&Next line. 2: Dispose a line when this line's note quantity <= Last&Next line.");
        public static readonly LocalisableString NoteAdjustLine_Label = new EzLocalisableString("线", "Line");
        public static readonly LocalisableString NoteAdjustLine_Description = new EzLocalisableString("取决于这张图的难度（0推荐用于Jack，1推荐用于（Jump/Hand/Etc.）Stream，2推荐用于Speed）", "Depending on how heavy about this map(0 is recommended for Jack,  1 is recommended for (Jump/Hand/Etc.)Stream, 2 is recommended for Speed).");
