...
INSERT INTO chart_analytics (
    path_key, pp, xxy_sr, avg_kps, max_kps, star_rating, column_counts_json, file_size, last_modified_ticks, scanned_at
) VALUES (
    $path_key, $pp, $xxy_sr, $avg_kps, $max_kps, $star_rating, $column_counts_json, $file_size, $last_modified_ticks, $scanned_at
)
ON CONFLICT(path_key) DO UPDATE SET
    ...
    scanned_at = excluded.scanned_at;";
        ...
        public bool TryGet(string pathKey, out BmsAnalyticsRecord record)
        {
            ...
            using var connection = openConnection();
            using var cmd = connection.CreateCommand();
            cmd.CommandText = "SELECT path_key, pp, xxy_sr, avg_kps, max_kps, star_rating, column_counts_json FROM chart_analytics WHERE path_key = $path_key LIMIT 1;";
        ...
        public IReadOnlyDictionary<string, BmsAnalyticsRecord> LoadAll()
        {
            ...
            using var connection = openConnection();
            using var cmd = connection.CreateCommand();
            cmd.CommandText = "SELECT path_key, pp, xxy_sr, avg_kps, max_kps, star_rating, column_counts_json FROM chart_analytics;";
