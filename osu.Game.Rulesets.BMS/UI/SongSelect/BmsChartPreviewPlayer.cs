...
namespace osu.Game.Rulesets.BMS.UI.SongSelect
{
    ...
    public partial class BmsChartPreviewPlayer : CompositeDrawable
    {
        ...
        private void scheduleAllSamples()
        {
            ...
            // Find the contiguous slice of samples that fall within [previewStartTime, horizonTime].
            for (int i = 0; i < timeline.Count; i++)
            {
                ...
                // Schedule a one-shot for each sample. Scheduler honours absolute delays accurately, so dense
                // BMS keysound charts no longer suffer the EzPreviewTrackManager 5ms-window drops.
                scheduledTriggers.Add(Scheduler.AddDelayed(() => triggerSample(captured), delay));
            }

            if (Loop)
            {
                // Set up a single loop-restart at horizonTime; we don't keep an active list-modifying timer.
