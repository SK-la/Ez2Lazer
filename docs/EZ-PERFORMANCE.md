...
- 剩余真实杠杆只有两个、且都与帧率无关：**GC**（见 §2.4.9：它占全部卡顿时间的约三分之一，而非「2.5% 墙钟」那么轻）与那个 **≈0.55 ms 输入地板**。

---

#### 2.4.9 GC 是卡顿帧的头号单一成因（2026-09-24，修正 §2.4.8 的表述）

§2.4.8 把 GC 记为「约占 2.5% 墙钟」——**分母用错了**。2.5% 是全程平均，会把它稀释掉；GC 的破坏力在于制造**偶发长帧**，正确的分母是「卡顿时间」。

方法：deep 模式 CSV 本身就是**只含 >1.5 ms stall 帧**的导出（`EZ_FRAME_PROBE_MS = 1.5`），且逐帧记了 `GcPauseDeltaMs`（本帧内 GC 暂停墙钟增量）。直接与 `ElapsedMs` 做相关即可，**不需要 profiler**。

| | A：base 500（1979 fps） | B：base 250（998 fps） |
|---|---|---|
| stall 帧数（>1.5 ms） | 538 | 998 |
| stall 帧总耗时 / 占墙钟 | 1454 ms / 4.3% | 2177 ms / **6.5%** |
| 其中 GC | 577 ms = **stall 时间的 39.7%** | 733 ms = **stall 时间的 33.7%** |
| 帧长 >2 ms | 227 | 356 |
| **其中伴随 GC（≥0.3 ms）** | **214（94.3%）** | **331（93.0%）** |
| 帧长 >5 ms 中伴随 GC | 10 / 14 | 31 / 32 |
| 有 GC 帧均值 vs 无 GC 帧均值 | 2.642 / 3.449 ms | 2.743 / 1.663 ms |

⇒ 帧长 >2 ms 的卡顿帧里 **93–94% 伴随 GC 暂停**，两局独立复现；GC 吃掉全部卡顿时间的约三分之一。**GC 是「下落不顺滑」的头号单一成因，不是边角项。**（B 组「无 GC 帧均值反而更高」是样本少 n=40 所致，不影响主结论。）

**与 `frameSplit` 合起来定位**：分配 92–93% 在 FSC 子树之外（`subtreeAllocMean` 仅 168 B / 270 B per frame，而 update 线程每帧 2552 B / 3162 B）⇒ **ruleset / 播放区不是分配源；目标是非 ruleset 的每帧分配**（HUD / 游戏层 / 框架）。

**已实测排除**：`EzBoxElement`（`AcrylicBackdropDrawable`）与 mania `Stage.stageBackdropBlur`（`BackdropBlurDrawable`，按 `ColumnBlur = 0.3` → `sigma = 15` 判定为**开启**）的毛玻璃。开关 Acrylic 对局内 draw 帧数无可见差别。**注意量具敏感度**：`FrameSync = Limit4x` 下 FPS 被锁在 1000，该数字本就不动，所以此结论只说明「acrylic 不显眼」，**不能**推广成「每帧分配不重要」——同一个不敏感的量具也照不出上面那 ~214 个 >2 ms 的帧。

**下一步**：在探针里把 update 线程分配再切一刀（`Player` 子树 / HUD vs 其余），把 2.5 KB/帧 落到具体模块后再决定改谁。
...
| 2026-09-24 | §2.4.9：**GC 归因定量（修正 §2.4.8 的分母）**——deep CSV 逐帧 `GcPauseDeltaMs` × `ElapsedMs` 直接相关，无需 profiler：帧长 >2 ms 的卡顿帧中 **93–94% 伴随 GC 暂停**，GC 占**全部 stall 时间的约 1/3**（A 39.7% / B 33.7%），故 GC 是「下落不顺滑」头号单一成因；配 `frameSplit`（分配 92–93% 在 FSC 之外）把目标定为**非 ruleset 的每帧分配**。同节登记 **acrylic 实测排除**（`EzBoxElement` / `Stage` 毛玻璃开关对 draw 帧数无可见差别，并注明限帧下量具不敏感） |
