...
跨探针侧最有信息的一条是 **`Drift × TimeOffset` 同时（分辨率内）r = +0.433** —— 两者锁相，
但先后分辨不出；能给出滞后数值的「内部峰」都出现在「帧」与「按键」之间（如
`FrameElapsed` 早 `FrameAgeMs` 520 ms，r = +0.571），33 s 样本且未见主周期，**不足以下结论**。
...
| 2026-09-24 | §2.4.10：**周期性分析工具 `AnalyzePeriod.py`**——补上 §2.1 起就缺的 ACF/谱/相位这条腿（此前那两个 ACF 数字无脚本、不可复现）。同一局三份 CSV 的 `WallMs` 同源故可跨探针做互相关。三条实测踩出的防误读自检：事件型序列的分辨率=采样间隔（该带内只报「同时」，但**不可排除**该段——排除后真峰会退到周期旁瓣，实测两个完全相同信号曾被报到 −1.06 s）、带通后相关峰很宽故**贴边即不可定**（只有内部峰给先后）、**尾部 frame 数据必须参与不了**（实测伪造 `TimeOffset × ElapsedMs r = −0.874`）。现有三局（各 ~34 s）**测不出 1.5–5 s 带内稳定周期**（`ACF r ≤ 0.20`，带内占比 ≤ 均匀背景 2.8 倍），与 §2.1 一致；结论待 §3 长时全帧捕获。另修正 §2.4 写错的开关名（总开关是 ini 的 `EzJudgmentDiagEnabled`，不是 `EZ_JUDGMENT_PROBE`） |
